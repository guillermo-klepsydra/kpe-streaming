...
using EventEmitterMF32A =
    kpsr::EventEmitterMiddlewareProvider<DataBatchWithId<std::vector<F32AlignedVector>>>;
...
{
public:
    EventEmitterPublishSubscribeFactoryFloat32(
...
private:
    std::shared_ptr<EventEmitterF32A> getEventEmitterF32A(const std::string &stepName,
                                                          const size_t vectorSize);
    std::shared_ptr<EventEmitterMF32A> getEventEmitterMF32A(const std::string &stepName,
                                                            const size_t vectorSize,
                                                            const size_t multiVectorSize);
    std::shared_ptr<EventEmitterF32> getEventEmitterF32(const std::string &stepName,
                                                        const size_t vectorSize);
